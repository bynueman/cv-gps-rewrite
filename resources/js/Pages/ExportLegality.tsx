...
function CertificationCard({
  ...
  return (
    <article className="group flex flex-col items-center gap-6 rounded-3xl border border-espresso-900/10 bg-white p-6 text-center transition-all duration-300 hover:-translate-y-1 hover:border-gold-300 hover:shadow-card sm:flex-row sm:items-start sm:p-8 sm:text-left">
      {/* Logo Area */}
      <div className="flex h-28 w-28 shrink-0 items-center justify-center rounded-2xl border border-espresso-900/5 bg-cream-50 p-4 transition-transform duration-500 group-hover:scale-105 sm:h-32 sm:w-32">
        {cert.logo ? (
          <img
            src={cert.logo}
            alt={cert.name}
          ...
          <ShieldCheck className="h-10 w-10 text-espresso-300" strokeWidth={1.5} aria-hidden="true" />
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col items-center sm:items-start w-full">
        <span
          className={`mb-3 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${colors.bg} ${colors.text}`}
        >
          <span className={`h-1.5 w-1.5 rounded-full ${colors.dot}`} aria-hidden="true" />
          {categoryLabel}
        </span>

        <h2 className="font-display text-xl font-bold leading-tight text-espresso-950">
          {cert.name}
        </h2>
        
        {cert.issuer ? (
          <p className="mt-1.5 text-sm font-medium text-espresso-500">{cert.issuer}</p>
        ) : null}

        <div className="mt-6 flex w-full flex-col items-center justify-between gap-4 border-t border-espresso-900/5 pt-4 sm:flex-row sm:items-end">
          {/* Date */}
          <div className="flex-1">
            {formattedDate ? (
              <div className="inline-flex items-center gap-1.5 text-xs text-espresso-500">
          ...
          {cert.pdf_url ? (
            <a
              ...
              className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-espresso-900/15 bg-cream-50 px-4 py-2 text-xs font-bold text-espresso-900 transition-colors hover:border-espresso-900 hover:bg-espresso-900 hover:text-white"
            >
